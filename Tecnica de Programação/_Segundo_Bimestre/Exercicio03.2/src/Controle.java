...
public class Controle {
    ...
    public void iniciaJogo() {
        ...
        resumoCompleto(this.jogadores, this.numeroSortido);
    }
    ...
    public void resumoCompleto(ArrayList<Jogador> jogadores, int numeroSortido) {
        String textoCompleto = "";
        String titulo = "RESUMO DE PALPITES, NÚMERO SORTIDO E POSSÍVEIS GANHADORES";

        textoCompleto += resumoDePalpites(jogadores, numeroSortido);
        textoCompleto += definirPossiveisGanhadores(jogadores, numeroSortido);

        JOptionPane.showMessageDialog(null, textoCompleto, titulo, 1);
    }
    
    public String resumoDePalpites(ArrayList<Jogador> jogadores, int numeroSortido) {
        String jogadoresEPalpites = "RESUMO DE PALPITES: "+numeroSortido+"\n\n";

        for (int i = 0; i < jogadores.size(); i++) {
            int[] palpites = jogadores.get(i).getPalpites();
            jogadoresEPalpites += "Jogador "+(i+1)+" - "+jogadores.get(i).getNome()+" - Palpites: ";
            for (int j = 0; j < palpites.length; j++) {
                jogadoresEPalpites += ", "+palpites[j];
            }
            jogadoresEPalpites += "\n\n";
        }

        return jogadoresEPalpites+"\n\n";
    }

    public String definirPossiveisGanhadores(ArrayList<Jogador> jogadores, int numeroSortido) {
